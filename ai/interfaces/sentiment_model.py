...
class MLP(object):
    ...
    def predict_test_data(self,testing_data):
        ...
        for tag in testing_data:
            print(self.tag_model[tag]['clf'].predict(
            self.tag_model[tag]['feature_gen'].transform(['Công  ty phát triển 10%'])))
        ...
        for tag in testing_data:
                # print(tag)
            corect[tag] = self.tag_model[tag]['clf'].predict(
                self.tag_model[tag]['feature_gen'].transform(test_data))
            score[tag] = self.tag_model[tag]['clf'].predict_proba(
                self.tag_model[tag]['feature_gen'].transform(test_data))
        ...
        for tag in label_predict:
            sum = 0
            for item in range(len(label_predict[tag])):
                if label_predict[tag][item]==tag:sum+=1
                total+=1
                if label_predict[tag][item]!=tag:
                    fail+=1
                    # print(tag,testing_data[tag][item],label_predict[tag][item])
